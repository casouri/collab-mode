...
async fn test_send_info_local_file() {
    // Test: Send info for a local file
    // Flow:
    // 1. Hub shares a file locally
    // 2. Spoke connects and opens the file
    // 3. Hub sends info for the file
    // 4. Spoke should receive InfoFromServer message
    let env = TestEnvironment::new().await.unwrap();
    let mut setup = setup_hub_and_spoke_servers(&env, 1, None).await.unwrap();
    ...
    // Wait for subscription to be established (spoke needs to send RequestOps first)
    ...
    // Spoke should receive InfoReceived notification
    let notification = setup.spokes[0]
        .editor
    ...
    // Verify the notification contains correct info
    assert_eq!(notification["file"], spoke_file_desc);

    // The info field contains the JSON string with the actual info data
    ...
    tracing::info!("test_send_info_local_file completed successfully");
    setup.cleanup();
}

#[tokio::test]
async fn test_send_info_remote_file() {
    // Test: Send info for a remote file
    // Flow:
    // 1. Hub shares a file
    // 2. Spoke opens the remote file
    // 3. Spoke sends info for the remote file
    // 4. Info should be forwarded to hub and broadcast to other subscribers
    let env = TestEnvironment::new().await.unwrap();
    let mut setup = setup_hub_and_spoke_servers(&env, 2, None).await.unwrap();
    ...
    // Spoke 2 should receive InfoReceived notification
    let notification = setup.spokes[1]
        .editor
        .wait_for_notification("InfoReceived", 5)
        .await
    ...
    tracing::info!("test_send_info_remote_file completed successfully");
    setup.cleanup();
}

#[tokio::test]
async fn test_send_info_multiple_subscribers() {
    // Test: Send info with multiple subscribers
    // Flow:
    // 1. Hub shares a file
    // 2. Multiple spokes open the file
    // 3. Hub sends info
    // 4. All spokes should receive the info
    let env = TestEnvironment::new().await.unwrap();
    ...
    for (i, spoke) in setup.spokes.iter_mut().enumerate() {
        ...
        spoke
            ...
            .await
            .unwrap();
        let _resp = spoke.editor.wait_for_response(req_id, 5).await.unwrap();
    ...
    for (i, spoke) in setup.spokes.iter_mut().enumerate() {
        ...
        // Verify each spoke received the correct info
    ...
    tracing::info!("test_send_info_multiple_subscribers completed successfully");
    setup.cleanup();
}
